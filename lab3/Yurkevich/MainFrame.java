...
public class MainFrame extends JFrame
{
    ...
    private JFileChooser fileChooser = null; //файл куда мы будет сохранять или открывать
    ...
    private MainFrame(Double[] coefficients)
    {
        ...
        // Создать новое "действие" по открытию "О прорамме"
        Action aboutAction = new AbstractAction("О программе")
        {
            public void actionPerformed(ActionEvent event)
            {
                JOptionPane.showMessageDialog(MainFrame.this, "9 группа, Юркевич Кирилл", "О программе", JOptionPane.INFORMATION_MESSAGE);
            }
        };
        ...
        buttonCalc.addActionListener(ev -> {
            try
            {
                ...
                data = new GornerTableModel(from, to, step, this.coefficients);
    ...
    private void saveToTextFile(File selectedFile)
    {
        try
        {
            ...
            for (int i = 0; i < data.getRowCount(); i++)
            {
                ...
                out.println("Значение больше нуля? - " + data.getValueAt(i, 2));
            }
            // Закрыть поток
            out.close();
        }
