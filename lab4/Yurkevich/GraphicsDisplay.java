package bsu.rfe.java.group9.lab4.Yurkevich;
...
// Шаг 2 - Организовать цикл по всем точкам графика
        for (Double[] point : graphicsData) {
            int value = point[1].intValue();
            double sq = Math.sqrt(value);
            // Выбрать красный цвета для контуров маркеров
            canvas.setColor((sq - Math.floor(sq)) == 0 ? Color.GREEN : Color.RED);
            GeneralPath marker = new GeneralPath();
            Point2D.Double pointCenter = xyToPoint(point[0], point[1]);

            marker.moveTo(pointCenter.getX() - 5.5, pointCenter.getY());
            marker.lineTo(pointCenter.getX() + 5.5, pointCenter.getY());
            marker.moveTo(pointCenter.getX(), pointCenter.getY() - 5.5);
            marker.lineTo(pointCenter.getX(), pointCenter.getY() + 5.5);

            marker.moveTo(pointCenter.getX() - 5.5, pointCenter.getY() - 1);
            marker.lineTo(pointCenter.getX() - 5.5, pointCenter.getY() + 1);
            marker.moveTo(pointCenter.getX() + 5.5, pointCenter.getY() - 1);
            marker.lineTo(pointCenter.getX() + 5.5, pointCenter.getY() + 1);
